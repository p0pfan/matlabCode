...
for i = 10 :tt
   ...
   A_train_out = A_measurement(i-5:i,1)';
    ...
    %%%%
    %train the model
    [A_train_predict,A_model] =lssvm_crossvalidate2(x_train_sample,A_train_out,test,test_out);
    [T_train_predict,T_model] =lssvm_crossvalidate2(y_train_sample,T_train_out,test,test_out);
%     [train_predict,model] =pso_lssvm(x_train_sample,train_out,test,test_out);
    %%%%
    %optimization
    xx_measurement = A_train_predict;
    yy_measurement= T_train_predict;
    if (i - 10) == 0
...
% %     init_matrix =  [yy_measurement 0 0 0 0 0 0]';
    A_init_matrix = [A_init_opt_value' 0 0 0 0 0 0]';
       T_init_matrix = [T_init_opt_value' 0 0 0 0 0 0]';
...
%     fval1 = lagrange_multiplier(Y1,yy_measurement,A_std,init_opt_value,sys_input,model,x_train_sample)
    options.Algorithm = 'levenberg-marquardt';
    Y1 = lsqnonlin(@(Y)lagrange_multiplier(Y,xx_measurement,A_std,A_init_opt_value,A_sys_input,A_model,x_train_sample),A_init_matrix,[],[],options)
     fval1 = lagrange_multiplier(Y1,xx_measurement,A_std,A_init_opt_value,A_sys_input,A_model,x_train_sample)
     A_reconciliation_value(i-5:i) = Y1(1 : 6);
     
     Y2 = lsqnonlin(@(YY)lagrange_multiplier(YY,yy_measurement,T_std,T_init_opt_value,T_sys_input,T_model,y_train_sample),T_init_matrix,[],[],options)
     fval2 = lagrange_multiplier(Y2,yy_measurement,T_std,T_init_opt_value,T_sys_input,T_model,y_train_sample)
